import 'package:bossapp/Compoments/CompanyListItem.dart';
import 'package:bossapp/Models/CompanyModel.dart';
import 'package:bossapp/screens/company/CompanyDetail.dart';
...
class _CompanyMainState extends State<CompanyMain> {
  List<CompanyModel> _companties = [];

  ///获取公司信息
  void getCompanyList() {
    setState(() {
      _companties = CompanyModel.fromJson("""
          {
            "list": [
               {
  ...
  buildCompanyItem(BuildContext context, int index) {
    CompanyModel company = _companties[index];

    var companyItem = new ListTile(
        onTap: () => navCompanyDetail(company, index),
        title: CompanyListItem(companyModel: company));

    return companyItem;
  }

  ///跳转详情页
  navCompanyDetail(CompanyModel company, int index) {
